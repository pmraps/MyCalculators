...
resourcestring
  ...
  rsDaysPeriod = ' days.';
  rsMonthPeriod = ' month.';
  rsMonthsPeriod = ' months.';
  rsMonthAnd = ' month and ';
  rsMonthsAnd = ' months and ';
  rsYearPeriod = ' year.';
  rsYearsPeriod = ' years.';
  rsYearAnd = ' year and ';
  ...
  rsHelpDescLine8 =
    'There is a small section where you can change the theme and/or the language, via menu Edit -> Settings.';
  //SIMPLE HELP STRINGS
  rsHelpSimpleLine0 =
    'This is the regular arithmetic calculator. Does basic functions, like adding, subtracting, multiplying and dividing.';
  rsHelpSimpleLine2 =
    'The remainder function (REMDR) gives the remainder of an integer division.';
  rsHelpSimpleLine4 =
    'You can add the present value to memory with M+, add another value with another M+, subtract the present value from whatever is in memory, or clear the memory.';
  rsHelpSimpleLine6 =
    'Be aware, if there is nothing in memory, with M- the present value will be stores as a negative value.';
  // MISCELLANEOUS FUNTIONS HELP STRINGS
  rsHelpMiscLine0 =
    'Here we have several commonly used functions and converters. Some require two arguments, most only one.';
  rsHelpMiscLine2 =
    '% and ‰ divide the number by 100 and 1000, respectively. x raised to y needs two numbers, the base (first number) and the exponent. As soon as you click the button, you will be redirected to the numeric keypad to enter the exponent.';
  rsHelpMiscLine4 =
    'Same thing happens with the nth root, exponential or logarythm functions. For the factorial, or the fraction of one, we need only one number.';
  rsHelpMiscLine6 =
    'For convenience, there are available trhee well known constants, Pi, Euler and Tau.';
  rsHelpMiscLine8 =
    'The four rightmost buttons pick the entered number and ask for the base to convert it to, assuming the number was entered in the base the button shows.';
  rsHelpMiscLine10 =
    'The other buttons are converters, they pick the first number and convert it as shown.';
  // TRIGONOMETRIC FUNCTIONS HELP STRINGS
  rsHelpTrigLine0 =
    'These are the most common trigonometric functions. Tehy pick the entered number and calculate the respective function.';
  rsHelpTrigLine2 =
    'The exception being the hypothenuse calculation (Hyp button), tha requires two operands.';
  rsHelpTrigLine4 =
    'There are also six converters between several angle measurement units.';
  // DATES HELP STRINGS
  rsHelpDateLine0 =
    'Here we can convert any date from one calendar to another. We do so by choosing a calendar using the first button. The default is the gregorian calendar. After that, we pick a date using the selector, and end with using the second button to select the target calendar.';
  ...
  rsHelpSettingsLine2 =
    'Click on the down arrow of the second list and select a theme, dark or light.';
  // COMMON BUTTONS
  rsStrBtnOK = '&OK';
  rsStrBtnClose = '&Close';
  rsStrBtnExit = 'Ex&it';
